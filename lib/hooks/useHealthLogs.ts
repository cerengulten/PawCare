...
import { HealthLog, PoopLogDetails } from '../../types';

type LogPayload = {
  details: PoopLogDetails;
  notes: string | null;
};

export function useHealthLogs(dogId: string | null) {
  ...
  async function addLog(payload: LogPayload) {
    ...
    const { data, error } = await supabase
      .from('health_logs')
      .insert({ dog_id: dogId, owner_id: user.id, type: 'poop', ...payload })
      .select()
      .single();
    if (!error && data) {
